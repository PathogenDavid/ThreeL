...
void ImGui_ImplDX12_RenderDrawData(ImDrawData* draw_data, ID3D12GraphicsCommandList* ctx)
{
    ...
    if (fr->VertexBuffer == nullptr || fr->VertexBufferSize < draw_data->TotalVtxCount)
    {
        ...
        if (bd->pd3dDevice->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&fr->VertexBuffer)) < 0)
            return;
        fr->VertexBuffer->SetName(L"Dear ImGui vertex buffer");
    }
    if (fr->IndexBuffer == nullptr || fr->IndexBufferSize < draw_data->TotalIdxCount)
    {
        ...
        if (bd->pd3dDevice->CreateCommittedResource(&props, D3D12_HEAP_FLAG_NONE, &desc, D3D12_RESOURCE_STATE_GENERIC_READ, nullptr, IID_PPV_ARGS(&fr->IndexBuffer)) < 0)
            return;
        fr->IndexBuffer->SetName(L"Dear ImGui index buffer");
    }
...
static void ImGui_ImplDX12_CreateFontsTexture()
{
    ...
    // Upload texture to graphics system
    {
        ...
        pTexture->SetName(L"Dear ImGui font atlas");
        ...
        uploadBuffer->SetName(L"Dear ImGui font atlas (Upload)");
        ...
        fence->SetName(L"Dear ImGui fence");
        IM_ASSERT(SUCCEEDED(hr));
        ...
        IM_ASSERT(SUCCEEDED(hr));
        cmdQueue->SetName(L"Dear ImGui command queue");

        ID3D12CommandAllocator* cmdAlloc = nullptr;
        hr = bd->pd3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&cmdAlloc));
        IM_ASSERT(SUCCEEDED(hr));
        cmdAlloc->SetName(L"Dear ImGui command allocator");

        ID3D12GraphicsCommandList* cmdList = nullptr;
        hr = bd->pd3dDevice->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, cmdAlloc, nullptr, IID_PPV_ARGS(&cmdList));
        IM_ASSERT(SUCCEEDED(hr));
        cmdList->SetName(L"Dear ImGui command list");
...
bool    ImGui_ImplDX12_CreateDeviceObjects()
{
    ...
    // Create the root signature
    {
        ...
        bd->pRootSignature->SetName(L"Dear ImGui root signature");
        blob->Release();
    }
    ...
    bd->pPipelineState->SetName(L"Dear ImGui pipeline state");
    vertexShaderBlob->Release();
...
static void ImGui_ImplDX12_CreateWindow(ImGuiViewport* viewport)
{
    ...
    vd->CommandQueue->SetName(L"Dear ImGui command queue (Secondary viewport)");
    IM_ASSERT(res == S_OK);
    ...
    for (UINT i = 0; i < bd->numFramesInFlight; ++i)
    {
        ...
        vd->FrameCtx[i].CommandAllocator->SetName(L"Dear ImGui command allocator (Secondary viewport)");
        IM_ASSERT(res == S_OK);
    }
    ...
    vd->CommandList->SetName(L"Dear ImGui command list (Secondary viewport)");
    IM_ASSERT(res == S_OK);
    vd->CommandList->Close();

    // Create fence.
    res = bd->pd3dDevice->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&vd->Fence));
    vd->CommandList->SetName(L"Dear ImGui fence (Secondary viewport)");
    IM_ASSERT(res == S_OK);
    ...
    if (vd->SwapChain)
    {
        ...
        IM_ASSERT(hr == S_OK);
        vd->RtvDescHeap->SetName(L"Dear ImGui RTV descriptor heap (Secondary viewport)");
